...
from F_taste_admin.kafka.kafka_consumer import consume


from F_taste_admin.ma import ma
...
from F_taste_admin.utils.jwt_custom_decorators import NoAuthorizationException
...
def create_app():
    ...
    app.config.from_object('F_taste_admin.config.DevelopmentConfig')
...
#    app.add_url_rule("/password_reset", view_func=TemplatePasswordChangerController.reindirizza, methods=['GET'])
 #   app.add_url_rule("/success", view_func=TemplatePasswordChangerController.successo, methods=['GET'])
  #  app.add_url_rule("/failure", view_func=TemplatePasswordChangerController.fallimento, methods=['GET'])
    ...
    def health_check():
        return {'message': 'API admin è online'}, 200
